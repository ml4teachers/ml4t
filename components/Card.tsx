  ...
  return (
    <div className={`z-30 absolute w-36 mr-12 h-24 border-2 border-gray-400 rounded-md cursor-pointer`} onClick={() => handleCardClick()}>
      <div className="relative h-full">
        ...
        <div className={`absolute top-7 left-3.5 w-8 h-10 rounded-sm text-lg leading-9 indent-3 text-gray-600 ${classes[0] === 1 ? 'hidden' : ''} ${colorClass}`}>0</div>
        ...
        <div className={`absolute top-7 ml-3.5 left-16 w-8 h-10 rounded-sm text-lg leading-9 indent-2.5 text-gray-600 ${classes[1] === 1 ? 'hidden' : ''} ${colorClass}`}>0</div>
        ...
        <div className={`absolute flex bottom-0 left-0 w-full h-[30px] rounded-b-md ${colorClass}`}>
          <div className="flex-auto text-xs mt-1 ml-3">{classes[0] === 1 ? 'Action' : ''}</div>
          <div className="text-xs text-right mr-5 mt-1">{classes[1] === 1 ? 'Comedy' : ''}</div>
        </div>
      </div>
    </div>
  );
